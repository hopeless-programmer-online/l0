    ...
    while (true) {
        ...
        switch (x) {
            ...
            case ':':
                ...
                break;
            case '(':
                yield skip1(RoundOpening);
                break;
            case ')':
                yield skip1(RoundClosing);
                break;
            case '[':
                yield skip1(SquareOpening);
                break;
            case ']':
                yield skip1(SquareClosing);
                break;
            case '{':
                yield skip1(CurlyOpening);
                break;
            case '}':
                yield skip1(CurlyClosing);
                break;
            case '\'':
            case '`':
            case '"':
    ...
    function transform(program, targets) {
        function transformCommands(commands, targets) {
            if (commands.length <= 0) {
                ...
                if (superIndex < 0)
                    throw new Error; // @todo
                const template = Template.from(superIndex + 1, // add 1 to compensate absence of "self" in targets
                selfIndex);
